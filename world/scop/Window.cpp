...
Window::Window(HINSTANCE instance, int nCmdShow, std::wstring const& class_name, int width, int height, std::wstring const& title)
{
	this->registerClass(instance, class_name);
	this->handle = this->initialize(instance, nCmdShow, class_name, title, width, height);
}
...
HWND	Window::initialize(HINSTANCE instance, int nCmdShow, std::wstring const& class_name, std::wstring const& title, int width, int height)
{
	LPCWSTR	window_class = reinterpret_cast<LPCWSTR>(class_name.c_str());
	...
	handle = CreateWindowW(window_class, window_title, WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, 0, rect.right - rect.left, rect.bottom - rect.top,
