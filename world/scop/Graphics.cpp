...
ComPtr<ID3D11Texture2D> Graphics::getSwapChainTexture() const
{
	ComPtr<ID3D11Texture2D> texture = nullptr;
	HRESULT hr;

	hr = this->swap_chain->GetBuffer(0, __uuidof(ID3D11Texture2D), reinterpret_cast<void**>(texture.GetAddressOf()));
	CHECK(hr);
	return (texture);
}
