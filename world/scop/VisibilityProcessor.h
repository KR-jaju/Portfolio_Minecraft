...
private:
    ...
    ComPtr<ID3D11ShaderResourceView> hi_z_srv;
    std::vector<ComPtr<ID3D11ShaderResourceView>> hi_z_srvs;
    std::vector<ComPtr<ID3D11UnorderedAccessView>> hi_z_uavs;
    ...
    ComPtr<ID3D11ComputeShader> occlusion_cs;
    ...
    void initializeOcclusionCullingData(ComPtr<ID3D11Device> const& device);
